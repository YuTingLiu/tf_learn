...
def plot_resized_image(image_path=None, image=None):
    if image is not None:
        # Image is passed in as a 3-dim array that is already decoded.
        resized_image = model.get_resized_image(image=image)
...
#    image_path = os.path.join(data_dir, 'cropped_panda.jpg')
    image_path = os.path.join(im_dir, '0.png')
    from PIL import Image
#    im = Image.open(image_path).convert('L')
    im = Image.open(image_path)
    data = np.asarray(im)
    plot_resized_image(image=data)
    # Use the Inception model to classify the image.
    pred = model.classify(image=data)
